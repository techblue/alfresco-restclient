...
public class ContentService extends AbstractService<ContentResource> {
	...
	public DocumentContent getNodeContent(String nodeProperty,
			String storeType, String storeId, String nodeId)
			throws ContentException {
		ClientResponse<DocumentContent> contentResponse = resourceProxy
				.getNodeContent(authTicket, nodeProperty, storeType, storeId,
						nodeId);
		return parseEntityFromResponse(contentResponse, ContentException.class);
	}

	public DocumentContent getNodeContent(String storeType, String storeId,
			String nodeId) throws ContentException {
		ClientResponse<DocumentContent> contentResponse = resourceProxy
				.getNodeContent(authTicket, "", storeType, storeId, nodeId);
		return parseEntityFromResponse(contentResponse, ContentException.class);
	}

	public DocumentContent getNodeContent(String nodeId)
			throws ContentException {
		ClientResponse<DocumentContent> contentResponse = resourceProxy
				.getNodeContent(authTicket, "",
						AlfrescoConstants.DEFAULT_STORE_TYPE,
						AlfrescoConstants.DEFAULT_STORE_ID, nodeId);
		return parseEntityFromResponse(contentResponse, ContentException.class);
	}
